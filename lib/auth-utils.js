...
export async function sendOTP(email, otp, type = 'EMAIL_VERIFICATION') {
  ...
  await prisma.verificationToken.create({
    data: { email: normalizedEmail, token: otp, expires, type }
  });
  ...
  let subject = 'Your Beauty Bees Cosmetics Verification Code';
  if (type === 'PASSWORD_RESET') subject = 'Reset Your Beauty Bees Cosmetics Password';

  try {
    await transporter.sendMail({
      from: `"Beauty Bees Cosmetics" <${process.env.GMAIL_USER}>`,
      to: email,
      subject,
      html: `
        <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 30px; border: 1px solid #f0e6e8; border-radius: 16px; background: #fffbfc;">
          <div style="text-align: center; margin-bottom: 24px;">
            ...
            <h2 style="color: #be185d; margin: 8px 0 0;">Beauty Bees Cosmetics</h2>
          </div>
...
export async function verifyOTP(email, token, type) {
  ...
  const record = await prisma.verificationToken.findFirst({
    where: { email: normalizedEmail, token, type }
  });
